function img= rotational2_inv_solve_diff_GN_one_step( inv_model, data1, data2)
...
% Modified to LAFR-rEIT and fast rotation (clicking mesh) by Olli Koskela, 2017
...
img.name= 'solved by rotational2_inv_solve_diff_GN_one_step';
img.elem_data = sol;
...
function RM = get_RM( inv_model )
   ...
   J = ClickingRotationalJacobian( inv_model, img_bkgnd );
...
function [img, step_size] = scale_to_fit_data(img, inv_model, data1, data2)
   ...
   if do_step
      eidors_msg('rotational2_inv_solve_diff_GN_one_step: Calculating optimal step size to fit data',2);
      % options for fminbnd
      try 
