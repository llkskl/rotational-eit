function img= rotational_inv_solve_diff_GN_one_step( inv_model, data1, data2)
...
% $Id: inv_solve_diff_GN_one_step.m 4833 2015-03-29 21:32:08Z bgrychtol-ipa $
...
% Modified to LAFR-rEIT by Olli Koskela, 2017

dv = calc_difference_data( data1, data2, inv_model.fwd_model);
% sol = eidors_cache(@get_RM, inv_model,'rotational_inv_solve_diff_GN_one_step' ) * dv;
sol = get_RM(inv_model) * dv(:);



img = data_mapper(calc_jacobian_bkgnd( inv_model ));
img.name= 'solved by rotational_inv_solve_diff_GN_one_step';
img.elem_data = sol;
...
function RM = get_RM( inv_model )
   ...
   J = RotationalJacobian( inv_model, img_bkgnd );
...
function [img, step_size] = scale_to_fit_data(img, inv_model, data1, data2)
   ...
   if do_step
      eidors_msg('rotational_inv_solve_diff_GN_one_step: Calculating optimal step size to fit data',2);
      % options for fminbnd
      try 
...
function [ J ] = RotationalJacobian( mdl, img )
%Computes jacobian that uses rotational mapping

    % first compute the normal jacobian
    J = calc_jacobian( img );

    % the mapping matrices are given in the input eidors model
    M = mdl.rotationalmapping;
    
    % we need to repeat the jacobian as block matrices on top of each other
    % for each rotational angle
    N = size(M,3);
    rJ = repmat(J,1,1,N);
    
    % block matrices (stacked above) are each modified to correspond the
    % rotation angle
    for i = 1:N
        rJ(:,:,i) = rJ(:,:,i)*M(:,:,i);
    end
